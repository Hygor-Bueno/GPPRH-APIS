...
function sqlUpdateJob(data, whereField = 'id') {
  const entries = verifyEntries(data).filter(([key]) => key !== whereField);
  const setClause = entries.map(([key]) => `${key} = ?`).join(', ');
  return `UPDATE jobs SET ${setClause} WHERE ${whereField} = ?`;
}

function buildUpdateParams(data, whereField = 'id') {
  const entries = verifyEntries(data).filter(([key]) => key !== whereField);
  return [...entries.map(([, value]) => value), data[whereField]];
}
...
function sqlOriginStatus() {
  return `SELECT status FROM jobs WHERE id = ?;`;
}
...
function sqlSelectJobApplication() {
  return `SELECT jb.* FROM
              gpprh.jobs jb
                  INNER JOIN
              gpprh.applications app ON jb.id = app.job_id
          WHERE
              app.candidate_id = ?
...
module.exports = { sqlInsertJob, sqlSelectJob, buildInsertParams, sqlUpdateJob, buildUpdateParams, sqlOriginStatus, spToggleJobLike, spToggleJobApplication, sqlInsertJobComments, sqlSelectJobComments, sqlSelectJobApplication };