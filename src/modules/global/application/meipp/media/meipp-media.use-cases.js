/**
 * @fileoverview Casos de uso de mídia (`meipp_media`).
 *
 * O binário não mora neste módulo: quem grava, deduplica e valida o arquivo é o
 * sistema `_files` do global, atrás do `MeippMediaStorageService`. Aqui só
 * decidimos *quando* chamá-lo e o que registrar na tabela.
 *
 * @module modules/global/application/meipp/media/meipp-media.use-cases
 */

const crypto = require('crypto');

const { AppError } = require('../../../../../errors/app.error');
const { MediaType, MediaStatus } = require('../../../domain/meipp/meipp.enums');
...
class MeippMediaUseCases {
     ...
     * @param {object} meippUser - autor (vai em `uploaded_by`).
     */
    async create(payload, file, meippUser) {
        ...
        const stored = await this.storage.save(file, meippUser?.global_user_id ?? null);
     ...
     *
     * `meipp_playlist_items` é ON DELETE CASCADE: sem esta checagem, apagar uma
...
module.exports = { MeippMediaUseCases, TYPES_REQUIRING_FILE };
