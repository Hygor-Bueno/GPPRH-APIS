...
const { MieppPlaylistUseCases } = require('../playlist/miepp-playlist.use-cases');
...
function makeUseCases(options = {}) {
    ...
    return {
        repository,
        useCases: new MieppPlaylistUseCases({
            repository,
...
describe('exclusão', () => {
    it('recusa quando há agendamento usando a playlist', async () => {
        // `fk_miepp_schedules_playlist` é ON DELETE CASCADE: sem esta guarda a
