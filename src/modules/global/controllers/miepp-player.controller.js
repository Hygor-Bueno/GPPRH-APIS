/**
 * @fileoverview Controller de players do miepp (lado do painel).
 *
 * @module modules/global/controllers/miepp-player.controller
 */

const { MieppPlayerUseCases } = require('../application/miepp/player/miepp-player.use-cases');
const { MysqlMieppPlayerRepository } = require('../infrastructure/miepp/mysql-miepp-player.repository');
const { pairingService } = require('../infrastructure/miepp/miepp-services');
const { respond } = require('../../../utils/respond');

const useCases = new MieppPlayerUseCases({
    repository: new MysqlMieppPlayerRepository(),
    pairingService,
});
...
async function enqueueCommand(req, res) {
    const result = await useCases.enqueueCommand(Number(req.params.id), req.body, req.user);
