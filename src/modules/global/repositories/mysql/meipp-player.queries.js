/**
 * @fileoverview Consultas SQL puras para `meipp_players`, `meipp_device_tokens`,
 * `meipp_device_commands` e `meipp_player_status_log`.
 *
 ...
 * dele.
 *
 * @module modules/global/repositories/mysql/meipp-player.queries
 */
 ...
 * varrendo `meipp_players` de tempos em tempos — mais peças móveis, e no
 ...
 *
 * ⚠️ O que isto NÃO faz: gravar o evento `offline` em `meipp_player_status_log`.
 * O log registra o que a tela reporta, e uma tela caída não reporta nada. Um
 * histórico de quedas exige o job varredor. Ver `docs/meipp.md`.
 *
...
const SQL_LIST_PLAYERS = `
    ...
    FROM meipp_players p
    LEFT JOIN meipp_locations l ON l.id = p.location_id
...
const SQL_FIND_DEVICE_TOKEN = `
    ...
    FROM meipp_device_tokens t
    INNER JOIN meipp_players p ON p.id = t.player_id
    WHERE t.token_hash = ?
...
const SQL_TOUCH_DEVICE_TOKEN = `
    UPDATE meipp_device_tokens SET last_used_at = NOW() WHERE id = ?
`;

const SQL_INSERT_DEVICE_TOKEN = `
    INSERT INTO meipp_device_tokens (player_id, token_hash, expires_at)
    VALUES (?, ?, ?)
`;

/** Revoga todos os tokens vivos do player — usado no pareamento e no revoke. */
const SQL_REVOKE_PLAYER_TOKENS = `
    UPDATE meipp_device_tokens
    SET revoked_at = NOW()
...
const SQL_INSERT_COMMAND = `
    INSERT INTO meipp_device_commands (player_id, command_type, payload, created_by)
    VALUES (?, ?, ?, ?)
`;

/**
 * Fila pendente do player. O device chama isso em loop, então o índice
 * `idx_meipp_device_commands_player_status` cobre exatamente este WHERE.
 */
const SQL_LIST_PENDING_COMMANDS = `
    SELECT id, command_type, payload, created_at
    FROM meipp_device_commands
...
const SQL_INSERT_STATUS_LOG = `
    INSERT INTO meipp_player_status_log (player_id, event_type, detail)
    VALUES (?, ?, ?)
`;

const SQL_LIST_STATUS_LOG = `
    SELECT id, event_type, detail, created_at
    FROM meipp_player_status_log
    WHERE player_id = ?
    ORDER BY created_at DESC
    LIMIT ? OFFSET ?
`;
...
const SQL_COUNT_STATUS_LOG = `
    SELECT COUNT(*) AS total FROM meipp_player_status_log WHERE player_id = ?
`;
