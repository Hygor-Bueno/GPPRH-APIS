 ...
 *
 * @module modules/global/repositories/mysql/gapp-vehicle.queries
 */
 ...
 * do cliente) — garante que não dá pra buscar por ID um veículo de outro
 * grupo de trabalho.
 */
function sqlGetVehicleById() {
    return `
        ...
        FROM global.gapp_vehicle v
        INNER JOIN global.gapp_active a ON v.active_id_fk = a.active_id
        WHERE v.vehicle_id = ? AND a.work_group_fk = ?
    `;
}
