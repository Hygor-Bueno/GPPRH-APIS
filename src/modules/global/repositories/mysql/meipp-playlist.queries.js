/**
 * @fileoverview Consultas SQL puras para `meipp_playlists` e `meipp_playlist_items`.
 *
 * @module modules/global/repositories/mysql/meipp-playlist.queries
 */
...
const SQL_LIST_PLAYLISTS = `
    ...
    FROM meipp_playlists pl
    LEFT JOIN meipp_playlist_items i ON i.playlist_id = pl.id
...
const SQL_INSERT_PLAYLIST = `
    INSERT INTO meipp_playlists (name, description, active, created_by)
    VALUES (?, ?, ?, ?)
`;

const SQL_UPDATE_PLAYLIST = `
    UPDATE meipp_playlists SET name = ?, description = ?, active = ? WHERE id = ?
`;

const SQL_DELETE_PLAYLIST = `
    DELETE FROM meipp_playlists WHERE id = ?
`;

/**
 * Quantos agendamentos usam a playlist. Guarda do DELETE: a FK
 * `fk_meipp_schedules_playlist` é ON DELETE CASCADE, então apagar a playlist
 * apagaria os agendamentos junto — e a tela pararia de tocar sem nenhum aviso.
 */
const SQL_COUNT_PLAYLIST_USAGE = `
    SELECT COUNT(*) AS total FROM meipp_schedules WHERE playlist_id = ?
`;
 ...
 * `domain/meipp/playlist/playlist-item.shaper` — os nomes das colunas do SELECT
...
const SQL_GET_PLAYLIST_ITEMS = `
    ...
    FROM meipp_playlist_items i
    INNER JOIN meipp_media m ON m.id = i.media_id
    WHERE i.playlist_id = ?
...
const SQL_SET_ITEM_ORDER = `
    UPDATE meipp_playlist_items SET order_index = ? WHERE id = ? AND playlist_id = ?
`;

/** Ids atuais da playlist — usado para validar o payload de reordenação. */
const SQL_GET_PLAYLIST_ITEM_IDS = `
    SELECT id FROM meipp_playlist_items WHERE playlist_id = ?
`;
