 ...
 *
 * @module modules/global/repositories/mysql/epp.queries
 */
...
const SQL_CHECK_PRODUCT_EXISTS = `SELECT id_product FROM epp_product WHERE id_product = ?`;

const SQL_CHECK_PRODUCT_LINKED_LOG_SALE = `
    SELECT epp_id_log FROM epp_log_sale WHERE epp_id_product = ? LIMIT 1
`;
...
// ─── Menus: Existência / Refetch ───────────────────────────────────────────────

const SQL_CHECK_MENU_EXISTS = `SELECT id_menu FROM epp_menu WHERE id_menu = ?`;

const SQL_CHECK_MENU_LINKED_LOG_MENUS = `
    SELECT epp_log_id FROM epp_log_menus WHERE epp_id_menu = ? LIMIT 1
`;

const SQL_GET_MENU_BY_ID = `
    SELECT id_menu, description, status FROM epp_menu WHERE id_menu = ?
`;

const SQL_GET_LOG_MENU_BY_ID = `SELECT * FROM epp_log_menus WHERE epp_log_id = ?`;
...
/** Resolve a loja do usuário via seu branch_code (Protheus) → global._shop. Parâmetros: [userId] */
...
const SQL_CHECK_ORDER_HAS_LOG_SALES = `
    SELECT epp_id_log FROM epp_log_sale WHERE epp_id_order = ? LIMIT 1
`;
...
const SQL_CHECK_ORDER_EXISTS = `SELECT id_order FROM epp_orders WHERE id_order = ?`;

const SQL_GET_LOG_SALE_BY_ID = `SELECT * FROM epp_log_sale WHERE epp_id_log = ?`;
...
const SQL_CHECK_STOCK_EXISTS = `SELECT id_stock FROM epp_stock WHERE id_stock = ?`;

const SQL_GET_STOCK_RAW_BY_ID = `SELECT * FROM epp_stock WHERE id_stock = ?`;
