/**
 * @fileoverview Consultas SQL puras para `meipp_schedules` e `meipp_schedule_targets`.
 *
 * @module modules/global/repositories/mysql/meipp-schedule.queries
 */
...
const SQL_LIST_SCHEDULES = `
    ...
    FROM meipp_schedules s
    INNER JOIN meipp_playlists pl ON pl.id = s.playlist_id
...
const SQL_COUNT_SCHEDULES = `
    ...
    FROM meipp_schedules
    WHERE (? IS NULL OR active = ?)
`;

const SQL_GET_SCHEDULE_BY_ID = `
    SELECT ${COLUMNS} FROM meipp_schedules WHERE id = ?
`;
...
const SQL_GET_ACTIVE_SCHEDULE_TARGETS = `
    ...
    FROM meipp_schedule_targets t
    INNER JOIN meipp_schedules s ON s.id = t.schedule_id
    INNER JOIN meipp_playlists pl ON pl.id = s.playlist_id
    WHERE s.active = 1 AND pl.active = 1
`;

const SQL_INSERT_SCHEDULE = `
    INSERT INTO meipp_schedules
...
const SQL_DELETE_SCHEDULE = `
    DELETE FROM meipp_schedules WHERE id = ?
`;
...
const SQL_GET_SCHEDULE_TARGETS = `
    ...
    FROM meipp_schedule_targets
    WHERE schedule_id = ?
    ORDER BY id
`;

const SQL_INSERT_SCHEDULE_TARGET = `
    INSERT INTO meipp_schedule_targets (schedule_id, target_type, target_id)
    VALUES (?, ?, ?)
`;

const SQL_DELETE_SCHEDULE_TARGET = `
    DELETE FROM meipp_schedule_targets WHERE id = ? AND schedule_id = ?
`;
 ...
 */
const SQL_PLAYER_EXISTS = `SELECT id FROM meipp_players WHERE id = ? LIMIT 1`;
const SQL_GROUP_EXISTS  = `SELECT id FROM meipp_player_groups WHERE id = ? LIMIT 1`;
