/**
 * @fileoverview Regra central do meipp: qual playlist um player deve tocar AGORA.
 *
 * É a decisão que o `GET /meipp/device/playlist` devolve a cada player, e a
 ...
 *
 * @module modules/global/domain/meipp/schedule/schedule-resolver.rules
 */

const { TargetType } = require('../meipp.enums');
 ...
 *
 * @param {object}   params
 * @param {object[]} params.schedules - linhas de `meipp_schedules` com `targets` embutidos.
 * @param {number}   params.playerId  - `meipp_players.id`.
 * @param {number[]} [params.groupIds] - grupos do player (`meipp_player_group_members`).
