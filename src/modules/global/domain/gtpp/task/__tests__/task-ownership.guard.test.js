...
describe('assertAnyOwnerOrAdmin', () => {
    ...
    it('should pass when not an owner but has an admin permission', () => {
        expect(() => assertAnyOwnerOrAdmin({ currentUserId: 99, ownerIds: [1], permissions: ['MANAGE_GTPP'] })).not.toThrow();
    ...
    it('should throw 403 when neither owner nor admin', () => {
        expect(() => assertAnyOwnerOrAdmin({ currentUserId: 99, ownerIds: [1], permissions: ['USE_GTPP'] })).toThrow(AppError);
    });
});
