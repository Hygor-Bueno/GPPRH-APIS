 ...
 * com validade curta, emitida no momento em que `GET /meipp/device/playlist`
 * monta a resposta.
 *
 * O token carrega o player: uma URL vazada só serve para o dispositivo a que
 * foi emitida, e some sozinha no vencimento.
 *
 * @module modules/global/infrastructure/meipp/meipp-media-token.service
 */
...
class MeippMediaTokenService {
    /**
     * @param {object} options
     * @param {string} options.secret     - `MEIPP_MEDIA_TOKEN_SECRET`.
     ...
     * chaves do meipp não pode impedir os outros módulos de subir. O erro
     * aparece na primeira rota do meipp que for usada, com o nome da variável.
     */
    _sign(payload) {
        if (!this.secret) {
            throw new AppError('MEIPP_MEDIA_TOKEN_SECRET não configurado.', 500);
        }
     ...
     *
     * @param {string} mediaUuid - `meipp_media.uuid`.
    ...
    buildUrl(mediaUuid, playerId) {
        ...
        return `${this.baseUrl}/meipp/media/${mediaUuid}/file?t=${token}&p=${playerId}`;
    }
...
module.exports = { MeippMediaTokenService, DEFAULT_TTL_HOURS };
