 ...
 * `meipp_pairing_codes` — é a extensão natural daqui.
 *
 * @module modules/global/infrastructure/meipp/meipp-pairing-code.service
 */
...
class MeippPairingCodeService {
    /**
     * @param {object} options
     * @param {string} options.secret - `MEIPP_PAIRING_SECRET`.
     ...
     * do `MeippMediaTokenService`: este processo serve outros módulos, e um
     * `.env` sem as chaves do meipp não pode derrubar o boot deles.
     */
    _sign(payload) {
        if (!this.secret) {
            throw new AppError('MEIPP_PAIRING_SECRET não configurado.', 500);
        }
     ...
     *
     * @param {string} code
     * @returns {number} `meipp_players.id`
     * @throws {AppError} 401
     */
    verify(code) {
...
module.exports = { MeippPairingCodeService, DEFAULT_TTL_MINUTES };
