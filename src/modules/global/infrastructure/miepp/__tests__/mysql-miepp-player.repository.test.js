...
const { MysqlMieppPlayerRepository } = require('../mysql-miepp-player.repository');
const {
    SQL_LIST_PLAYERS,
...
/** @returns {MysqlMieppPlayerRepository} */
const makeRepository = () => new MysqlMieppPlayerRepository({ offlineAfterMinutes: OFFLINE_MINUTES });
...
describe('default da tolerância', () => {
    it('sem injeção, usa o valor do config/miepp', async () => {
        const { mieppConfig } = require('../../../../../config/miepp');
        await new MysqlMieppPlayerRepository().findById(1);

        expect(helper.query).toHaveBeenCalledWith(
            SQL_GET_PLAYER_BY_ID,
            [mieppConfig.offlineAfterMinutes, 1]
        );
    });
});
