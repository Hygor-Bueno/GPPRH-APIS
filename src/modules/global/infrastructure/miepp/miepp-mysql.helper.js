/**
 * @fileoverview Acesso ao `poolGlobal` compartilhado pelos adapters do miepp.
 *
 ...
 *
 * @module modules/global/infrastructure/miepp/miepp-mysql.helper
 */
...
const ERROR_CODE = 'MIEPP_MYSQL_ERROR';

/**
 * Executa uma leitura no `poolGlobal`.
 *
 * Usa `pool.query()` e não `pool.execute()`, acompanhando o resto dos adapters
 * do módulo global. A diferença importa aqui: as listagens do miepp usam
...
async function query(sql, params = []) {
    try {
        ...
        throw new AppError('Erro ao acessar o banco de dados do miepp.', 500, {
            code: ERROR_CODE,
            details: error,
        });
    }
...
async function execute(sql, params = []) {
    try {
        ...
        if (error instanceof AppError) throw error;
        throw new AppError('Erro ao gravar no banco de dados do miepp.', 500, {
            code: ERROR_CODE,
            details: error,
        });
    }
