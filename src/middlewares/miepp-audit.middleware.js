 ...
 * `miepp_audit_log`, centralizado — e não repetido em cada controller. É isto:
 ...
 * conhece, ao mesmo tempo, o autor (`req.user`), o que foi pedido
 ...
 *
 * @module middlewares/miepp-audit.middleware
 */

const { MysqlMieppAuditRepository } = require('../modules/global/infrastructure/miepp/mysql-miepp-audit.repository');

const repository = new MysqlMieppAuditRepository();
 ...
 * segredo ali anularia o cuidado de guardar só o hash em `miepp_device_tokens`.
 */
        ...
        res.json = (body) => {
            ...
            if (res.statusCode >= 200 && res.statusCode < 300) {
                repository.record({
                    ...
                    console.error(
                        `[miepp] falha ao gravar auditoria (${entityType}):`,
                        error.message
                    );
                });
            }
