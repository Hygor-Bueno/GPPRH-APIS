/**
 * @fileoverview Schemas de validação das rotas do miepp.
 *
 ...
 *
 * As listas vêm de `domain/miepp/miepp.enums` — uma fonte só para o ENUM do
 * banco e a validação do payload, que é o que impede as duas de divergirem.
 *
 * @module schemas/miepp.schema
 */

const { LocationType, HardwareType, Orientation,
    MediaType, MediaStatus, Transition, TargetType, CommandType,
    CommandStatus, StatusLogEvent, values,
} = require('../modules/global/domain/miepp/miepp.enums');
const { ALL_DAYS } = require('../modules/global/domain/miepp/schedule/days-of-week');
...
const postMediaSchema = {
    ...
    // 500 = tamanho de `miepp_media.file_id`, que é onde a URL externa fica
    // gravada (ver `miepp-deploy.sql`).
